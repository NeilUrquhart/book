...
public class NonDomEA extends VRPSolver {
	...
	public void solve() {
		...
		for (int count=0; count < INIT_POP_SIZE; count++){
			MObjTWIndividual i = new MObjTWIndividual(theProblem);
			i.evaluate();
			init.add(i);
		}
		...
		while(evalsBudget >0) {	
			for (int count =0; count < CHILDREN; count ++){
				...
				if (rnd.getRnd().nextDouble() < XO_RATE){
					...
					child = new MObjTWIndividual(super.theProblem, (MObjTWIndividual) population.getDominator(),(MObjTWIndividual) population.getDominator());				
				}
				else{
					//Create a child by copying a single parent
					child =  ((MObjTWIndividual) population.getDominator()).copy();
				}
				child.mutate();
				child.evaluate();
				evalsBudget --;
